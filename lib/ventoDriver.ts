...
export default class VentoDriver extends Driver {
  ...
  async locateDevices(): Promise<{ id: string }[]> {
    ...
    const existingDevices: { id: string }[] = this.getDevices().map((d) => ({
      id: d.getData().id,
    })); // We want to be able to tell any non initialized devices they are ready for use
    this.log(
      `Current we located ${existingDevices.length} devices, lets see if we found more: amount located ${locatedDevices.length}`
    );

    locatedDevices.forEach((locatedDevice) => {
      const hasDevice = existingDevices.find(
