...
export default class Api {
  ...
  private readonly setDeviceValue = async (param: Parameter, value: number) =>
    this.send(
      new Packet(this.deviceId, this.devicePass, FunctionType.WRITE, [
        DataEntry.of(param, value),
      ]),
      this.deviceIp
    );

  setOnOffStatus = async (value: number) =>
    this.setDeviceValue(Parameter.ON_OFF, value);

  setSpeedMode = async (value: number) =>
    this.setDeviceValue(Parameter.SPEED, value);

  setOperationMode = async (value: number) =>
    this.setDeviceValue(Parameter.VENTILATION_MODE, value);

  setTimerMode = async (value: number) =>
    this.setDeviceValue(Parameter.TIMER_MODE, value);

  setManualSpeed = async (value: number) =>
    this.setDeviceValue(Parameter.MANUAL_SPEED, value);

  setHumiditySensor = async (value: number) =>
    this.setDeviceValue(Parameter.HUMIDITY_SENSOR_ACTIVATION, value);

  setHumiditySensorThreshold = async (value: number) =>
    this.setDeviceValue(Parameter.HUMIDITY_THRESHOLD, value);

  setBoostDelay = async (value: number) =>
    this.setDeviceValue(Parameter.BOOST_MODE_DEACTIVATION_DELAY, value);
}
